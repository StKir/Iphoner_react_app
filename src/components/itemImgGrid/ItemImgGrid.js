    ...
    return(
        <section className="photo_iphons">
            <div className="container">
                ...
                {(process === 'success')? (<GridTypeOne gridType={gridType} arrayImg={imgArray}/>): <h1>Ошибка при загрузке картинок</h1>}
            </div>
        </section>
    )
}
...
const GridTypeOne = ({arrayImg,gridType}) => {
    const grid = arrayImg.map(({urls, id}, index) => {
        return (
            <img src={urls.regular} key={id} alt="iphone img" className={`img-${index}`} />
        )
    })
    return (
        <div className={`grid-one_img_wrp gridType-${gridType}`}>
            {grid}
        </div>
    )
}
